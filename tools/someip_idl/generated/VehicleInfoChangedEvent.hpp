...
struct VehicleInfoChangedEvent : public someip_generated::SerializableObject {
    ...
    bool serializeTo(std::vector<std::uint8_t>& bytes) const override {
        ...
        if (!writer.writeIntegralBE<std::uint32_t>(eventSequence)) return false;
        if (!status.serializeTo(bytes)) return false;
        return true;
    }

    bool deserializeFromReader(someip_generated::ByteReader& reader) override {
        if (!reader.readIntegralBE<std::uint32_t>(eventSequence)) return false;
